...
// Safely apply fills to SVG string with extensive error handling
// In utils.js, replace the applyFillsToSvgString function with:
...
export const applyFillsToSvgString = (svgString, fills) => {
  ...
  try {
    console.log("Applying fills to SVG:", Object.keys(fills));
    ...
    if (svg.nodeName === "parsererror") {
      console.error("Error parsing SVG:", svg.textContent);
      return svgString;
    }
    ...
    Object.entries(fills).forEach(([selector, color]) => {
      try {
        if (selector === 'main-element') {
          // Try different approaches to find a fillable element
          // First try to find the most obvious fillable element
          const fillable = doc.querySelector('path, circle, rect, ellipse, polygon');
          if (fillable) {
            fillable.setAttribute('fill', color);
            console.log(`Applied ${color} to main element (found ${fillable.tagName})`);
          } else {
            // If no standard shape element, try the SVG itself or any group
            const rootOrGroup = doc.querySelector('svg, g');
            if (rootOrGroup) {
              rootOrGroup.setAttribute('fill', color);
              console.log(`Applied ${color} to ${rootOrGroup.tagName} as fallback`);
            }
          }
        } else if (selector.includes('-')) {
          // Handle tag-index format
          ...
          if (elements && elements.length > index) {
            ...
            console.log(`Applied ${color} to ${tagName}-${index}`);
          }
        } else {
          // Try by ID first
          const elementById = doc.getElementById(selector);
          if (elementById) {
            elementById.setAttribute('fill', color);
            console.log(`Applied ${color} to element by ID: ${selector}`);
          } else {
            // Try by tag name
            ...
            if (elements && elements.length > 0) {
              ...
              console.log(`Applied ${color} to all ${selector} elements`);
            } else {
              // Try by CSS selector as last resort
              const elementBySelector = doc.querySelector(selector);
              if (elementBySelector) {
                elementBySelector.setAttribute('fill', color);
                console.log(`Applied ${color} to element by CSS selector: ${selector}`);
              }
            }
          }
        }
      } catch (error) {
        console.error(`Error applying color to ${selector}:`, error);
      }
    });
    
    // If there's a root SVG element with no fill, also color it
    if (svg && !svg.hasAttribute('fill') && Object.keys(fills).some(key => key.includes('svg'))) {
      svg.setAttribute('fill', Object.values(fills)[0]);
      console.log("Applied fill to root SVG element");
    }
    ...
    console.error('Error applying fills to SVG:', error);
    return svgString; // Return original if error
  }
};
